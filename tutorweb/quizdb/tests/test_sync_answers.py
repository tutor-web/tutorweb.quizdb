...
class GetCoinAwardTest(FunctionalTestCase):
    ...
    def test_registeredstudent(self):
        """Get super awards if registered to at least one class"""
        aqTime = [1400000000]
        def aqEntry(alloc, qnIndex, correct, grade_after, user=USER_A_ID):
            qnData = self.getJson(alloc[qnIndex]['uri'], user=user)
            aqTime[0] += 10
        ...
        portal = self.layer['portal']
        lecObj1 = portal['dept1']['tut1']['lec1']
        lecObj2 = portal['dept1']['tut1']['lec2']
        self.objectPublish(lecObj1)
        self.objectPublish(lecObj2)

        dbLec1 = getDbLecture('/'.join(lecObj1.getPhysicalPath()))
        dbLec2 = getDbLecture('/'.join(lecObj2.getPhysicalPath()))
        dbA = getDbStudent(USER_A_ID)
        dbB = getDbStudent(USER_B_ID)

        def gca(dbStudent, grade_after, settings={}, gradeHighWaterMark=0):
            """Call getCoinAward with some mocks"""
            dbQn = db.Question()
            dbAnsSummary = db.AnswerSummary(
                gradeHighWaterMark=gradeHighWaterMark,
            )
            a = dict(
                grade_after=grade_after,
            )
            return getCoinAward(dbLec1, dbStudent, dbAnsSummary, dbQn, a, settings)

        registered_bonus = dict(
            award_registered_lecture_answered="2000",
            award_registered_lecture_aced="20000",
            award_registered_tutorial_aced="200000",
        )
        # By default, we get the default awards
        self.assertEqual(gca(dbA, 0, registered_bonus), 0)
        self.assertEqual(gca(dbA, 5, registered_bonus), 1000)
        self.assertEqual(gca(dbA, 9.8, registered_bonus), 11000)
        self.assertEqual(gca(dbB, 0, registered_bonus), 0)
        self.assertEqual(gca(dbB, 5, registered_bonus), 1000)
        self.assertEqual(gca(dbB, 9.8, registered_bonus), 11000)

        # Add class with B in it
        login(portal, MANAGER_ID)
        ...
        setRelations(classObj, 'lectures', [
        ...
        self.assertEqual(gca(dbA, 5, registered_bonus), 1000)
        self.assertEqual(gca(dbA, 9.8, registered_bonus), 11000)
        self.assertEqual(gca(dbB, 5, registered_bonus), 2000)
        self.assertEqual(gca(dbB, 9.8, registered_bonus), 22000)
        # ...only if the settings are set
        self.assertEqual(gca(dbA, 5, {}), 1000)
        self.assertEqual(gca(dbA, 9.8, {}), 11000)
        self.assertEqual(gca(dbB, 5, {}), 1000)
        self.assertEqual(gca(dbB, 9.8, {}), 11000)

        # B aces lecture 2, now gets full tutorial award
        login(portal, USER_B_ID)
        bAllocs = list(self.allocGetQuestionAllocation(dbLec2, dbB, {}))
        import transaction ; transaction.commit()
        self.allocParseAnswerQueue(dbLec2, dbB, [
            aqEntry(bAllocs, 0, True, 9.9, user=USER_B_ID),
        ], {})
        self.assertEqual(gca(dbB, 9.8, {}), 111000)
        self.assertEqual(gca(dbB, 9.8, registered_bonus), 222000)
